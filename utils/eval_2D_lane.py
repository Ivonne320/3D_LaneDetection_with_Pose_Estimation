...
"""this code is to modify the original eval_3D_lane.py to evaluate 2d lanes"""
...
class LaneEval():
    def __init__(self):        
        ...
        ######## normal perspective view, to be updated with image size ######## 
        # self.x_min = args.view_region[0, 0]
        self.x_min = 0
        # self.x_max = args.view_region[1, 0]
        self.x_max = 1920
        # self.y_min = args.view_region[2, 1]
        self.y_min = 0
...
########## All visible in 2D lane ground truth ##########
        ...
        #     if lane is not None:
        #         print("lane_shape:", np.array(lane).shape)
        #     else:
        #         print("Lane is None")
        ...
        gt_lanes = [prune_2d_gt_lane_by_range(lane, self.x_min, self.x_max) for lane in gt_lanes]
        ...
        gt_visibility_mat = np.zeros((cnt_gt, len(self.y_samples)))
        pred_visibility_mat = np.zeros((cnt_pred, len(self.y_samples)))

        # resample gt and pred at y_samples
        for i in range(cnt_gt):
            min_y = np.min(np.array(gt_lanes[i])[:, 1])
            max_y = np.max(np.array(gt_lanes[i])[:, 1])
            x_values, visibility_vec = resample_2dlaneline_in_y(np.array(gt_lanes[i]), self.y_samples, out_vis=True)
            # gt_lanes[i] = np.vstack([x_values, z_values]).T
            gt_lanes[i] = np.array(x_values)
        ...
        for i in range(cnt_pred):
            ...
            min_y = np.min(np.array(pred_lanes[i]['keypoints'])[1::3])
            max_y = np.max(np.array(pred_lanes[i]['keypoints'])[1::3])
            x_values, visibility_vec = resample_2dpred_laneline_in_y(np.array(pred_lanes[i]['keypoints']), self.y_samples, out_vis=True)
            # pred_lanes[i] = np.vstack([x_values, z_values]).T
            pred_lanes[i] = np.array(x_values)
        ...
        for i in range(cnt_gt):
            for j in range(cnt_pred):
                ...
                x_dist = np.abs(np.array(gt_lanes[i]) - np.array(pred_lanes[j]))

                # z_dist = np.abs(gt_lanes[i][:, 1] - pred_lanes[j][:, 1])

                # apply visibility to penalize different partial matching accordingly
                both_visible_indices = np.logical_and(gt_visibility_mat[i, :] >= 0.2, pred_visibility_mat[j, :] >= 0.2)
                both_invisible_indices = np.logical_and(gt_visibility_mat[i, :] < 0.2, pred_visibility_mat[j, :] < 0.2)
                other_indices = np.logical_not(np.logical_or(both_visible_indices, both_invisible_indices))
                
                euclidean_dist = np.sqrt(x_dist ** 2)
                ...
                if np.sum(both_visible_indices[:close_range_idx]) > 0:
                    # print("shape of x_dist[:close_range_idx]: ", x_dist[:close_range_idx].shape)
                    # print("shape of both_visible_indices[:close_range_idx]: ", both_visible_indices[:close_range_idx].shape)  
        ...
        # if vis:
        #     pass 
        return r_lane, p_lane, c_lane, cnt_gt, cnt_pred, match_num, x_error_close, x_error_far

####### which gt to pass to the evaluator?########
    ...
    def bench_one_submit(self, pred_dir, gt_dir, test_txt, prob_th=0.5, vis=False):
        ...
        for i, pred in enumerate(json_pred):
            if i % 50 == 0 or i == len(json_pred)-1:
    ...
    def bench_one_submit_openlane_DDP(self, pred_lines_sub, gt_lines_sub, prob_th=0.5, vis=False):
        ...
        for i, pred in enumerate(json_pred):
            ...
            #     print('eval:{}/{}'.format(i+1,len(json_pred)))
            # if 'file_name' not in pred[0] or 'laneLines' not in pred[0]:
            #     raise Exception('file_path or lane_lines not in some predictions.')
            ...
            #     else:
            #         pred_category = []
            pred_category = [item['category_id'] for item in pred_lanes]

######### To do: include pred_category and pred_lane_prob ######### 
            if raw_file not in gts:
                raise Exception('Some raw_file from your predictions do not exist in the test tasks.')
            gt = gts[raw_file]
            ...
            for j, gt_lane_packed in enumerate(gt_lanes_packed):
                # A GT lane can be either 2D or 3D
                # if a GT lane is 3D, the height is intact from 3D GT, so keep it intact here too
                lane = np.array(gt_lane_packed['uv'])
                lane_visibility = np.array(gt_lane_packed['visibility'])
####### get rid of camera transformation since we are looking at image space directly ########
                # lane = np.vstack((lane, np.ones((1, lane.shape[1]))))
                # cam_representation = np.linalg.inv(
                #                         np.array([[0, 0, 1, 0],
                #                                   [-1, 0, 0, 0],
