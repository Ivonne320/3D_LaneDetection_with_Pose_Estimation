...
class Visualizer:
    ...
    def save_result_new(self, dataset, train_or_val, epoch, idx, images, gt, pred, pred_cam_pitch, pred_cam_height,
                        aug_mat=np.identity(3, dtype=np.float32), evaluate=False,
...
def prune_2d_gt_lane_by_range(lane_2d, x_min, x_max):
    # remove lane points out of x range
    lane_2d = lane_2d[np.logical_and(lane_2d[:, 0] > x_min,
                                     lane_2d[:, 0] < x_max), ...]
    return lane_2d
def prune_2d_lane_by_range(lane_2d, x_min, x_max):
    ...
    if isinstance(lane_2d, np.ndarray):
        lane_2d = lane_2d[mask, ...]
    else:
        # if lane_2d is not a numpy array, you can apply mask to the keypoints list itself
        lane_2d_np = np.array(lane_2d['keypoints']).reshape(-1,3)
        lane_2d_np = lane_2d_np[mask, ...]
        # valid_indices = np.where(mask)[0] * 3  # Convert back to original indices
...
def resample_2dlaneline_in_y(input_lane, y_steps, out_vis=False):
    assert(input_lane.shape[0] >= 2)
    y_min = np.min(input_lane[:, 1])-5
    y_max = np.max(input_lane[:, 1])+5
    # input_lane = input_lane[input_lane[:,1].argsort()]
    f_x = interp1d(np.array(input_lane[:, 1]), np.array(input_lane[:, 0]), fill_value="extrapolate")
    # f_x = interp1d(input_lane[:, 1], input_lane[:, 0],bounds_error=False, fill_value=None)
    x_values = f_x(y_steps)
    # x_values[y_steps < y_min] = np.NaN
    # x_values[y_steps > y_max] = np.NaN
    if out_vis:
        output_visibility = np.logical_and(y_steps >= y_min, y_steps <= y_max)
        return x_values, output_visibility.astype(np.float32) + 1e-9
    return x_values
